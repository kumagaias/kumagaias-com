...
export default function ParkScene({ attractions, placingType, onPlace, onBalloonPop, demolishing, onDemolish, shops, placingShopType, onPlaceShop, onDemolishShop, onWeatherChange, celebrateTriggerRef, currentVisitors, capacity, effectiveCapacity }: Props) {
  ...
  useEffect(() => {
    ...
    const attractionGroups = attractionGroupsRef.current;
    const shopGroups = shopGroupsRef.current;
    const pathMeshes = pathMeshesRef.current;
  ...
  const infoPanelContent = (() => {
    ...
    if (infoPanel.kind === "attraction") {
      const a = attractions.find(a => a.id === infoPanel.id);
      if (!a) return null;
      ...
      const s = shops.find(s => s.id === infoPanel.id);
      if (!s) return null;
